...
exports.sendQuote = async (req, res) => {
  try {
    ...
    if (quote.artistId.toString() !== req.user.id) {
...
exports.acceptQuote = async (req, res) => {
  try {
    ...
    const quote = await Quote.findById(id).populate('artistId', 'name email');
    if (!quote) {
      return res.status(404).json({
        success: false,
        message: 'Devis non trouvé'
    ...
    try {
      ...
      if (!clientProfile) {
        clientProfile = new ClientProfile({
          clientId: quote.clientId,
          artistId: quote.artistId._id,
          name: quote.clientInfo?.name || 'Client',
        ...
        if (!clientProfile.projects.includes(project._id)) {
          clientProfile.projects.push(project._id);
...
exports.rejectQuote = async (req, res) => {
  try {
    ...
    quote.rejectionReason = reason || 'Aucune raison spécifiée';
    quote.updatedAt = new Date();
    await quote.save();
    ...
    const pdfContent = `
      <html>
        ...
        <body>
          ...
          <div class="section">
            <h3>Artiste</h3>
            <p>${quote.artistId?.name || 'Artiste'}</p>
            <p>${quote.artistId?.email || ''}</p>
          </div>
          ...
          <div class="section">
            ...
            ${quote.services.map(service => `
              <p>${service.name || service} - ${service.price || 0}€</p>
            `).join('')}
          </div>
...
exports.getQuote = async (req, res) => {
  try {
    ...
    if (quote.artistId._id.toString() !== req.user.id && 
